...
def projecao_pesquisa(banco, projetado):
    ...
    while True:
        setor_final = input('Nome arquivo Distribuição: ').strip()  # pedir nome usuario
        if os.path.isfile(setor_final):  # verifica se existe arquivo
            break
        print('Nome de arquivo inexistente, tente outra vez ou digite exit para sair')
        if setor_final.upper() == 'EXIT':
            exit()

    distribuicao = pd.read_excel(setor_final, 'Plan2')
    cont = 0
    pulo = 0  # numero de linhas pula em cada setor na projeção
    resultado = pd.DataFrame(columns=banco.columns)  # dataframe com colunas
    ...
    for nome_setor, valor in setor.iteritems():
        ...
        if nome_setor == distribuicao.SETOR[cont]:
            falta = int(int(distribuicao.TOTAL[cont] * projetado) - int(valor))
            if falta == 0:
                print(f'Setor {nome_setor} com quantidade correta')
                cont += 1
                continue
            elif falta > 0:
                ...
                if pulo == 1 and valor % falta != 0:
                    ...
                    print('Numero de entrevistas insuficiente '
                          'para projetar setor ', nome_setor)
                    exit()
            elif falta < 0:
                print(f'Setor {nome_setor} está acima do numero '
            ...
            cont_resto = 1
            print(nome_setor, resto)
            print((distribuicao.TOTAL[cont] * projetado), valor, falta)
            ...
            print('Problema na ordem do setor.')
        cont += 1

    frames = [banco, resultado]
    resultado = pd.concat(frames, ignore_index=True)
    resultado['qt_campo'] = banco.shape[0]
    nome_pla = input('Nome arquivo projetado: ')
    writer = pd.ExcelWriter(nome_pla)
    resultado.to_excel(writer, 'Plan1')
    writer.save()
    banco_novo = pd.read_excel(nome_pla, 'Plan1')
    return banco_novo, nome_pla
...
def arruma_variaveis_sexo(banco, pesq_campo):
    ...
    menor_sexo = int(floor(0.5050 * banco.shape[0]))  # numero de pessoas do sexo feminino
    maior_sexo = int(0.54 * banco.shape[0])
    print(sexo[0])

    sexo_alterado = randint(menor_sexo, maior_sexo)  # define valor do FEMININO
    pergunta = 'SEXO'

    if sexo.FEMININO > sexo_alterado:  # Entra se tiver muito Feminino
        alter_mudar = 'FEMININO'
        alter_corrente = 'MASCULINO'
        diferenca = sexo.FEMININO - sexo_alterado  # diferença a ser alterada
        print(sexo[0], sexo_alterado, diferenca)
        mudanca.mudar_alternativa(banco=banco, pesq_campo=pesq_campo,
                                  pergunta=pergunta, alter_corrente=alter_corrente,
                                  alter_mudar=alter_mudar, diferenca=diferenca,
        ...
        sexo_alterado = banco.shape[0] - sexo_alterado  # define valor masculino
        diferenca = sexo.MASCULINO - sexo_alterado  # diferença a ser alterada
...
def arrumar_variaveis_idade(banco, pesq_campo):
    ...
    idade_31_alterado = 400 - (idade_16_alterado + idade_50_alterado)  # calcula 31 que falta
    ...
    if idade['31 a 50'] < idade_31_alterado:
        '''se 31 a 50 faltando, verifica qual opção tem acima e transfere aleatoriamente'''
...
def arrumar_variaveis_escolaridade(banco, pesq_campo):
    ...
    if escolaridade['ENSINO MÉDIO'] < esco_med_altereada:
        '''se medio faltando, verifica qual opção tem acima e transfere aleatoriamente'''
            ...
            for variaveis in lista_variaveis:
                ...
                for altervar in lista_altervar:
                    ...
                    cruz_falta = list(banco[(banco[pergunta] == alterperg) &
                                            (banco[
                                                 variaveis] == altervar)].index)  # armazena indice de alternativa sem certo cruzamento
                    # print(cruz_falta)
